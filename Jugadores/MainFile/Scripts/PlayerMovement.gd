...
func Update(_delta: float):
	...
	if(get_parent().get_parent().position.y >= 800):
		get_parent().get_parent().get_node("StateMachine").get_node("DeadState")._on_vida_componente_dead_player()
